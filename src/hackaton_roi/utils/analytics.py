...
def create_dashboard(data):
    ...
    if any(data['executive_summary']['initial_conditions_summary'].values()):
        ...
        business_type = data['executive_summary']['initial_conditions_summary']['business_type']
        if business_type:
        ...
        target = data['executive_summary']['initial_conditions_summary']['target_audience_estimate']
        if target:
            # Use a container with text instead of metric to avoid text cutting
            with col3.container():
    ...
    if any(data['executive_summary']['explanation_of_key_terms'].values()):
        with st.expander("Key Financial Terms"):
            for term, definition in data['executive_summary']['explanation_of_key_terms'].items():
                if definition:
    ...
    direct_competitors = data['market_and_competitive_analysis']['competitors_analysis']['direct_competitors']
    indirect_competitors = data['market_and_competitive_analysis']['competitors_analysis']['indirect_competitors']
                    ...
                    if comp['name']:
                        with st.expander(comp['name']):
                            ...
                            if comp['user_business_differentiation']:
                                st.write(f"**Differentiation**: {comp['user_business_differentiation']}")
    ...
    with col1:
        ...
        init_inv_data = data['investment_structure_and_operational_costs']['initial_investment']['breakdown_by_items']
        if any(init_inv_data.values()):
            ...
            # Display total
            total = data['investment_structure_and_operational_costs']['initial_investment']['total_estimated']
            if total:
                st.metric("Total Initial Investment", f"${total:,}")
            
            # Display consultant notes
            notes = data['investment_structure_and_operational_costs']['initial_investment']['consultant_notes']
            if notes:
    ...
    with col2:
        ...
        # Check if there's data to display
        monthly_costs = data['investment_structure_and_operational_costs']['monthly_operational_costs']['breakdown_by_items']
        if any(monthly_costs.values()):
            ...
            # Display total
            total = data['investment_structure_and_operational_costs']['monthly_operational_costs']['total_estimated']
            if total:
                st.metric("Total Monthly Costs", f"${total:,}")
            
            # Display consultant notes
            notes = data['investment_structure_and_operational_costs']['monthly_operational_costs']['consultant_notes']
            if notes:
                st.info(f"**Consultant Notes**: {notes}")
    ...
    yearly_data = data['financial_projections_break_even_and_ROI']['3_year_financial_projection']['yearly_projections']
    if any(yearly_data):
    ...
    break_even_time = data['financial_projections_break_even_and_ROI']['break_even_analysis']['estimated_break_even_time']
    if break_even_time:
        # Use a container with text instead of metric to avoid text cutting
        with col1.container():
    ...
    payback_period = data['financial_projections_break_even_and_ROI']['ROI_and_payback_analysis']['estimated_payback_period']
    if payback_period:
        # Use a container with text instead of metric to avoid text cutting
        with col2.container():
    ...
    licenses = data['compliance_regulation_and_expansion']['required_licenses_and_regulations']
    if any(licenses):
            ...
            if license_item['license_or_regulation']:
                with st.expander(license_item['license_or_regulation']):
                    ...
                    if license_item['how_to_obtain_or_comply']:
                        st.write(f"**How to Obtain/Comply**: {license_item['how_to_obtain_or_comply']}")
    
    # International expansion
    expansion_data = data['compliance_regulation_and_expansion']['international_expansion_opportunities']
            ...
            if source['source_name']:
                with st.expander(f"{i}. {source['source_name']}"):
                    ...
                    if source['methodology_if_estimate']:
                        st.write(f"**Methodology**: {source['methodology_if_estimate']}")
